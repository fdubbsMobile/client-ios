        ...
        switch (tagType) {
            ...
            case FTCoreTextTagTypeClose:
            {
                ...
                if (currentSupernode.isLink) {
                    ...
                    if (img) {
                        img = [self scaleImage:img toWidth:-1];
	...
	for (int i = 0; i < lineCount; i++) {
        ...
        if (cfrange.location > imageNode.styleRange.location) {
			...
			if (img) {
                ...
                img = [self scaleImage:img toWidth:self.frame.size.width];
                ...
                NSLog(@"x is %d, lineFrame.origin.y is %f", x, lineFrame.origin.y);
				CGRect frame = CGRectMake(x, (lineFrame.origin.y - img.size.height), img.size.width, img.size.height);
                
                NSLog(@"before FRAME.X = %f, Y=%f", frame.origin.x, frame.origin.y);
                // adjusting frame
                ...
                NSLog(@"after FRAME.X = %f, Y=%f", frame.origin.x, frame.origin.y);
...
- (UIImage *)scaleImage:(UIImage *)image toWidth:(CGFloat)scaleWidth
{
    NSLog(@"scaleImage");
    ...
    if (scaleWidth < 0) {
        ...
        scaleWidth = screenRect.size.width;
    }
    
    float scaleSize = scaleWidth / image.size.width;
